...
def test_merge_uses_discord_as_source_of_truth(store):
    """Merging = linking both accounts to one member: refuses across two
    members, adopts the linked side's owner, requires @member when unlinked."""
    ...
    finish = cog._merge_finish(None)
    # neither account linked -> a member is required
    assert "rerun with one" in finish("H-rain1", "H-rain2", "t1", "t2")

    # one side linked -> the other adopts that owner
    store.bind_specific("discX", "Rain", "H-rain1")
    assert "share one rating" in finish("H-rain1", "H-rain2", "t1", "t2")
    assert set(store.handles_for("discX")) == {"H-rain1", "H-rain2"}

    # accounts owned by two different members -> refused
    store.add_account("discY", "H-A2")
    assert "Refusing" in finish("H-rain1", "H-A2", "t1", "t2")

    # explicit member contradicting an existing owner -> refused
    bob = SimpleNamespace(id=999, display_name="Bob")
    finish_bob = cog._merge_finish(bob)
    assert "Refusing" in finish_bob("H-rain1", "H-B1", "t1", "t2")

    # explicit member, both unlinked -> both linked to that member
    assert "share one rating" in finish_bob("H-B1", "H-B2", "t1", "t2")
    assert set(store.handles_for("999")) == {"H-B1", "H-B2"}
